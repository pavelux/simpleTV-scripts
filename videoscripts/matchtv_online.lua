-- видеоскрипт для сайта https://matchtv.ru (9/11/20)
...
-- https://matchtv.ru/on-air
		...
		if m_simpleTV.Control.CurrentAddress:match('PARAMS=match_tv') then return end
	...
	local t, i = {}, 1
		for w in answer:gmatch('{.-}') do
			...
			if adr and name and res then
				...
				t[i].Name = name
				t[i].Address = adr .. '$OPT:INT-SCRIPT-PARAMS=match_tv'
				i = i + 1
			end
		end
